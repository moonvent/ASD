...
class BinaryTree:
    ...
    def insert_node(self, value):  # добавление в дерево
        if value < self.value and self.left_child:  # если есть дерево
    ...
    def pre_order(self, ls, height, max):  # Предварительный обход (сверху вниз)
        ...
        if self.left_child:
            self.left_child.pre_order(ls, height, max)

        if self.right_child:
            self.right_child.pre_order(ls, height, max)
